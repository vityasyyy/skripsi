...
\node[box, draw=okgreen, fill=okgreen!10, right=0.3cm of s1] (s2) {
    \textbf{2. Baseline}\\
    $O_1$: Kalibrasi
};
\node[box, draw=driftred, fill=driftred!10, right=0.3cm of s2] (s3) {
    \textbf{3. Injeksi}\\
    Skenario A--G
};

\node[box, draw=metricyellow!80!black, fill=metricyellow!10, below=0.5cm of s1] (s4) {
    \textbf{4. Ukur}\\
    $O_2$: R1, R2, R3
};
\node[box, draw=manualgray, fill=manualgray!10, below=0.5cm of s2] (s5) {
    \textbf{5. Analisis}\\
    Mann--Whitney U
};
\node[box, draw=gitopsblue, fill=gitopsblue!10, below=0.5cm of s3] (s6) {
...
\draw[looparrow] (s4.west) to[out=180, in=180, looseness=1.5]
    node[left, font=\tiny, text=driftred!80!black] {Iterasi ($n$ kali)} (s2.west);
