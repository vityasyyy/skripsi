...
\begin{tikzpicture}[
    ...
    metricbox/.style={rectangle, rounded corners=5pt, draw=#1, fill=white,
                      minimum width=4.2cm, minimum height=3.8cm, align=center,
                      line width=1.5pt},
    metriclabel/.style={rectangle, rounded corners=3pt, draw=#1, fill=#1,
                        text=white, minimum width=4.0cm, minimum height=0.8cm,
                        align=center, font=\small\bfseries, line width=1pt},
    metricdesc/.style={font=\scriptsize, align=left, text width=3.6cm},
...
% R2
\node[metriclabel=driftred, right=1.2cm of r1label] (r2label) {R2: Waktu (detik)};
...
\node[metricdesc, anchor=north west] at ($(r2box.north west)+(0.2,-0.2)$) {
    ...
    Recovery Time ($t_2 - t_0$)\\
    Root Cause ID ($t_{rci} - t_d$)\\[4pt]
    \textbf{Pengulangan:}\\
    $n = 10$ per skenario
};

% R3
\node[metriclabel=metricpurple, right=1.2cm of r2label] (r3label) {R3: Traceability};
\node[metricbox=metricpurple, below=0pt of r3label] (r3box) {};
\node[metricdesc, anchor=north west] at ($(r3box.north west)+(0.2,-0.2)$) {
    \textbf{Pertanyaan:}\\
    Apakah perubahan dapat\\
    direkonstruksi?\\[6pt]
    \textbf{Cara Ukur:}\\
    \% \textit{audit log} yang\\
    dapat direkonstruksi\\[6pt]
    \textbf{Catatan:}\\
    Deskriptif, bukan statistik
};
