...
\node[svc] (web) {Web};
\node[svc, right=0.4cm of web] (worker) {Worker};
\node[svc, below=0.38cm of web] (pg) {PostgreSQL};
\node[svc, right=0.4cm of pg] (os) {OpenSearch};
\node[svc, below=0.38cm of pg] (redis) {Redis};
\node[svc, right=0.4cm of redis] (minio) {MinIO};
...
\node[infra, right=1.2cm of worker] (ingress) {Ingress};
\node[infra, right=0.4cm of ingress] (cert) {Cert-manager};
\node[infra, below=0.38cm of ingress] (sealed) {Sealed Secrets};
\node[infra, right=0.4cm of sealed] (monitor) {Monitoring};
\node[infra, below=0.38cm of $(sealed)!0.5!(monitor)$] (backup) {Backup};

\begin{scope}[on background layer]
    \node[rectangle, rounded corners=3pt, draw=gitopsblue, fill=gitopsblue!5,
          inner sep=9pt, line width=0.8pt, fit=(web)(worker)(pg)(os)(redis)(minio)] (svcgrp) {};
    \node[rectangle, rounded corners=3pt, draw=manualgray, fill=manualgray!5,
          inner sep=9pt, line width=0.8pt, fit=(ingress)(cert)(sealed)(monitor)(backup)] (infragrp) {};
\end{scope}

\node[font=\normalsize\bfseries, above=0.2cm of svcgrp.north] {Layanan};
\node[font=\normalsize\bfseries, above=0.2cm of infragrp.north] {Infrastruktur};

\coordinate (topcenter) at ($(svcgrp.south)!0.5!(infragrp.south)$);
\node[criteria, below=0.7cm of topcenter] (c1) {
    \textbf{(a)} Kompleksitas dependensi $\rightarrow$ urutan deployment
};
\node[criteria, below=0.22cm of c1] (c2) {
    \textbf{(b)} 16+ komponen infrastruktur yang saling bergantung
};
\node[criteria, below=0.22cm of c2] (c3) {
    \textbf{(c)} Open-source, dapat direproduksi
};

\node[rectangle, rounded corners=3pt, draw=driftred, fill=driftred!10,
      inner sep=7pt, line width=1pt, below=0.3cm of c3,
