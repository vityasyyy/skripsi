...
Penelitian empiris oleh \citet{zhang2026configuration} menunjukkan bahwa 71\% dari 2.260 skrip Kubernetes yang dianalisis mengandung \textit{misconfiguration} yang bermakna, di mana sebagian besar menyebabkan \textit{downtime} atau degradasi layanan. Meskipun demikian, literatur akademis masih jarang menemukan studi kuasi-eksperimental yang secara sistematis membandingkan dampak operasional dari pendekatan GitOps dengan pendekatan manual pada metrik yang terdefinisi dengan jelas.

GitOps menawarkan disiplin yang berpotensi mengatasi masalah ini. Dengan menjadikan repositori Git sebagai \textit{single source of truth}, seluruh \textit{state} infrastruktur tercatat, terverifikasi, dan dapat diaudit \citep{opengitops2021}. ArgoCD mengimplementasikan prinsip-prinsip GitOps dengan menjalankan agen di dalam klaster yang secara periodik membandingkan \textit{state} Git dengan \textit{state} aktual klaster, dan secara otomatis menyelaraskan keduanya \citep{argocd2024}. \citet{kaggantinataraja2025security} menemukan bahwa pendekatan deklaratif (GitOps/ArgoCD) menghasilkan \textit{drift correction} yang lebih cepat, \textit{compliance rate} yang lebih tinggi, dan paparan kerentanan yang lebih rendah dibandingkan pendekatan imperatif (Jenkins/\texttt{kubectl}).

Namun, temuan-temuan tersebut masih bersifat komparatif pada level konseptual atau dilakukan pada skala kecil. Belum ada studi kuasi-eksperimental yang secara eksplisit mengukur dan membandingkan --- dengan protokol statistik yang jelas --- empat aspek operasional berikut: (1)~konsistensi deployment setelah \textit{drift}; (2)~waktu deteksi dan pemulihan \textit{drift}; (3)~jumlah intervensi manual yang diperlukan; dan (4)~tingkat jejak audit (\textit{traceability}).

Kesenjangan tersebut menjadi landasan penelitian ini, yang mengevaluasi secara empiris dampak GitOps menggunakan ArgoCD terhadap \textit{configuration drift} pada sebuah workload multi-service (InvenioRDM) yang di-deploy pada klaster Kubernetes.
...
\begin{enumerate}
  ...
  \item \textbf{Variabel Dependen:} Empat metrik utama --- (1)~\textit{konsistensi deployment} (persentase), (2)~\textit{waktu deteksi dan pemulihan drift} (detik), (3)~\textit{jumlah intervensi manual} (jumlah tindakan korektif), dan (4)~\textit{tingkat jejak audit / traceability} (skor rubrik 1--5 serta Mean Time To Identify Drift / MTTI).
  ...
  \item Analisis statistik menggunakan uji Shapiro-Wilk untuk normalitas, dilanjutkan dengan \textit{paired t-test} (jika data normal) atau \textit{Wilcoxon signed-rank test} (jika data tidak normal), dengan tingkat signifikansi $\alpha = 0,05$.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item Mengukur dan membandingkan tingkat jejak audit (\textit{traceability}) pada lingkungan GitOps dan lingkungan manual menggunakan rubrik operasional.
\end{enumerate}
...
\subsection{Hipotesis Alternatif (H$_1$)}
...
\begin{itemize}
  ...
  \item \textbf{Bab II:} Penelitian Terkait --- membahas studi empiris terkait \textit{configuration drift} pada Kubernetes, evaluasi GitOps, metode eksperimental pada riset sistem, serta penelitian terdahulu yang relevan.
  \item \textbf{Bab III:} Metode dan Rancangan Penelitian --- menjelaskan metodologi studi kuasi-eksperimen, definisi variabel, tujuh skenario drift, protokol pengukuran, analisis statistik, dan lingkungan pengembangan.
