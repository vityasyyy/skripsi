...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
   ...
   box/.style={
    ...
    driftbox/.style={
      rectangle, draw, rounded corners=3pt, minimum width=2.4cm,
      minimum height=0.85cm, align=center, font=\small,
      fill=red!15
    },
    metricbox/.style={
      rectangle, draw, dashed, rounded corners=3pt, minimum width=1.6cm,
  ...
  \node[box, fill=orange!15] (operator) [below=1.3cm of git] {Operator\\(Peneliti)};

  \node[box, fill=gray!15] (kubectl) [below left=1.0cm and 0.6cm of operator] {\texttt{kubectl}\\(Imperatif)};

  \node[clusterbox] (clusterA) [below=1.0cm of kubectl] {Klaster K8s\\Lingkungan A};

  \node[box, fill=blue!10] (argocd) [below right=1.0cm and 0.6cm of operator] {ArgoCD\\(\textit{Controller})};

  \node[clusterbox] (clusterB) [below=1.0cm of argocd] {Klaster K8s\\Lingkungan B};

  \node[driftbox] (drift) [left=1.8cm of operator] {Injeksi Drift\\(Skenario A--G)};

  \node[metricbox] (metricsA) [left=0.4cm of clusterA] {R1--R4};

  \node[metricbox] (metricsB) [right=0.4cm of clusterB] {R1--R4};

  \draw[arr] (git) -- (argocd) node[midway, right, font=\scriptsize] {watch};
  \draw[arr] (operator) -- (kubectl);
  \draw[arr] (operator) -- node[midway, above, font=\scriptsize] {push} (git);
  \draw[arr] (kubectl) -- (clusterA);
  \draw[arr] (argocd) -- (clusterB) node[midway, right, font=\scriptsize] {sync};
  \draw[arr] (clusterB.west) -- ++(-0.4,0) |- (argocd.south east) node[pos=0.25, right, font=\scriptsize] {\textit{reconcile}};
  \draw[dasharr] (drift) -- (clusterA) node[midway, below, font=\scriptsize] {\texttt{kubectl edit}};
  \draw[dasharr] (drift) -- (operator);
  \draw[dasharr] (drift.south) |- (clusterB.west) node[pos=0.7, below, font=\scriptsize] {\texttt{kubectl edit}};
  \draw[dasharr] (metricsA) -- (clusterA);
  \draw[dasharr] (metricsB) -- (clusterB);

  \begin{scope}[on background layer]
    \node[fit=(kubectl)(clusterA)(metricsA), draw=gray, dashed, rounded corners=6pt, inner sep=8pt, label={[font=\scriptsize\bfseries, gray]above:Lingkungan A (Manual)}] {};
    \node[fit=(argocd)(clusterB)(metricsB), draw=blue!50, dashed, rounded corners=6pt, inner sep=8pt, label={[font=\scriptsize\bfseries, blue!70]above:Lingkungan B (GitOps)}] {};
  \end{scope}
\end{tikzpicture}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    node distance=0.7cm,
    stage/.style={
      rectangle, draw, rounded corners=4pt, minimum width=4cm,
      minimum height=0.8cm, align=center, font=\small, text width=3.8cm
    },
    arr/.style={-Stealth, thick},
  ...
  \draw[looparr] (s4.east) -- ++(1.3,0) |- (s2.east)
    node[pos=0.25, right, font=\scriptsize, text width=2cm, align=left] {Iterasi per\\skenario (acak)};

  \node[font=\scriptsize, text width=2.3cm, align=center, left=0.6cm of s3] (driftlabel) {7 skenario drift\\(A--G) diacak\\urutannya};
