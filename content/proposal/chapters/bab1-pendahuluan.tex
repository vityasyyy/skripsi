...
\begin{enumerate}
  ...
  \item Kelas \textit{workload} yang digunakan adalah \textit{workload} komposit multi-service, yang direpresentasikan oleh deployment InvenioRDM beserta komponen infrastruktur dan dependensinya. InvenioRDM berfungsi sebagai subjek uji (\textit{test subject}), bukan objek penelitian; temuan berlaku untuk kelas \textit{workload} multi-service secara umum.
...
Tujuan utama dari penelitian yang diuraikan dalam proposal ini adalah merekomendasikan paradigma manajemen \textit{deployment} yang sesuai bagi praktisi industri berdasarkan bukti empiris mengenai keunggulan salah satu paradigma (\textit{manual} atau GitOps) dalam menghadapi penyimpangan konfigurasi pada klaster Kubernetes. Untuk mencapai tujuan tersebut, dilakukan pengukuran dan perbandingan terkontrol sebagai metode pemerolehan bukti, yang mencakup:
...
\begin{enumerate}
  ...
  \item Praktis: Menyediakan dasar rekomendasi berbasis bukti kuantitatif yang membantu praktisi industri dalam memilih paradigma \textit{deployment} yang sesuai, serta menyoroti skenario penyimpangan yang paling rentan pada masing-masing paradigma.
\end{enumerate}
...
\begin{itemize}
  ...
  \item Bab II: Penelitian Terkait, membahas studi empiris terkait penyimpangan konfigurasi pada Kubernetes, evaluasi GitOps, dasar teori \textit{reconciliation} dan \textit{convergence}, metode eksperimental pada riset sistem, serta \textit{research gap} yang menjadi landasan penelitian.
